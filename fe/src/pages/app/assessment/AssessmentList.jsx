...
import { FiCheck, FiEye, FiFileText, FiSearch, FiX, FiChevronLeft, FiChevronRight, FiEdit, FiFilePlus } from "react-icons/fi";
...
const AssessmentList = () => {
  ...
  const [showUploadApl, setShowUploadApl] = useState(null);
  ...
  // Membuka dialog detail apl
  const handleButtonUpdateApl = (assessment) => {
    setCurrentAssessment(assessment);
    setShowUploadApl(true);
  };
  ...
  // button lengkapi apl
  const buttanLengkapi = (status, item) => {
    const isDisabled = status?.toLowerCase() === 'completed';

    const handleClick = () => {
      const currentStatus = status?.toLowerCase();
      if (currentStatus === 'approved' || currentStatus === 'rejected') {
        navigate(`/user/new_upload_apl/${item.id}`);
      } else {
        handleButtonUpdateApl(item);
      }
    };
    ...
    return (
      ...
      >
        <FiFilePlus className="h-4 w-4" /> Lengkapi Berkas
      </Button>
    );
  };
  
  
  // ketika dokuemnt belum di setujui
  const renderStatusBadgelg = (status) => {
    const baseClass = "text-lg px-3 py-1.5 rounded-md";
  
    if (!status) return (
      <Badge variant="outline" className={`${baseClass} bg-gray-100 text-gray-800`}>
        Belum Diproses
      </Badge>
    );
    ...
    switch (status.toLowerCase()) {
      case 'pending':
        return (
          <Badge variant="outline" className={`${baseClass} bg-yellow-100 text-yellow-800`}>
            Menunggu Verifikasi
          </Badge>
        );
      case 'rejected':
      ...
      case 'approved':
        return (
          <Badge variant="outline" className={`${baseClass} bg-blue-100 text-blue-800`}>
            Disetujui
          </Badge>
        );
      case 'completed':
        return (
          <Badge variant="success" className={`${baseClass} bg-green-100 text-green-800`}>
            Selesai
          </Badge>
        );
      default:
  ...
  return (
    <div className="container mx-auto p-4">
                ...
                {currentItems.map((item) => (
                  <TableRow key={item.id} className="hover:bg-gray-50">
                    ...
                    <TableCell className="text-right">
                      ...
                      {buttanLengkapi(item.assessment_status, item)}
                        ...
                        <Button
                          variant="default"
                          size="sm"
                          disabled={userRole === 'admin' && item.assessment_status !== 'approved'}
      ...
      {/* Modal upload apl */}
      <Dialog open={showUploadApl} onOpenChange={setShowUploadApl}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>Upload Berkas APL setelah status disetujui</DialogTitle>
            <DialogDescription>
              Informasi lengkap tentang pengajuan asesmen
            </DialogDescription>
          </DialogHeader>
          ...
          {currentAssessment && (
            <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-bold">Status berkas saat ini</h3>
                <h3>{renderStatusBadgelg(currentAssessment.assessment_status)}</h3>
              </div>
            </div>
          )}
          ...
          <DialogFooter>
            <Button 
              type="button" 
              variant="outline" 
              onClick={() => setShowUploadApl(false)}
            >
              Tutup
            </Button>
            {userRole === 'admin' && currentAssessment && currentAssessment.assessment_status !== 'completed' && (

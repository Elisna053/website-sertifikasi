...
const EditAssessment = () => {
    ...
    const fetchAssessment = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          setInstanceId(result.data.instance_id); // trigger fetch jadwal
          const formattedDate = result.data.assessment_date?.split('T')[0] || '';
  ...
  useEffect(() => {
    if (!assessment || jadwal.length === 0) return;
  
    const formattedDate = assessment.assessment_date?.split('T')[0];
  
    if (formattedDate && jadwal.some(j => j.date === formattedDate)) {
      setFormData(prev => ({
        ...prev,
        assessment_date: formattedDate,
      }));
  ...
  useEffect(() => {
    const fetchJadwal = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${API_URL}/instances-jadwal-reference/${instanceId}`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json'
          }
        ...
        if (response.ok && result.success && result.data) {
          ...
          console.log("Jadwal dates:", result.data.map(j => j.date));
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Terjadi kesalahan saat memperbarui data");
  ...
  const handleSelectJadwalChange = (name, value) => {
    setFormData(prev => ({
      ...prev,
      [name]: value
    }));
  ...
  const handleSupportingDocumentAdd = (e) => {
    ...
    newFiles.forEach(file => {
      if (!allowedTypes.includes(file.type)) {
        toast.error(`File "${file.name}" tidak didukung. Silakan unggah file dalam format PDF, JPG, JPEG, atau PNG.`);
        validFiles = false;
        return;
      }
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  onValueChange={(value) => handleSelectChange('assessment_date', value)}
                  disabled={jadwal.length === 0}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={jadwal.length === 0 ? "Tidak ada jadwal tersedia" : "Pilih tanggal asesmen"} />
                  </SelectTrigger>
                  <SelectContent>
                    {jadwal.map((item) => (
